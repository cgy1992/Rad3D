...
// this coder from spot image
//	See: https://github.com/r-lyeh/spot
//
...
#define PVR_FLAG_TYPE_MASK 0xFF
	...
	bool PVR_Load(Image & image, DataStreamPtr stream)
	{
		...
		image.format = header.bitmaskAlpha ? ePixelFormat::PVRTC_RGBA : ePixelFormat::PVRTC_RGB;
	...
	byte * PVR_Decode(const byte * blocks, int width, int height, ePixelFormat format)
	{
		d_assert (format == ePixelFormat::PVRTC_RGB || format == ePixelFormat::PVRTC_RGBA);

		byte * pixels = new byte[width * height * 4];

		Decompress((AMTC_BLOCK_STRUCT *)blocks, false, width, height, 1, pixels);

		return pixels;
	}

	void PVR_DecodeBlock(byte pixels[16*4], const byte * iblock, ePixelFormat format)
	{
		d_assert (format == ePixelFormat::PVRTC_RGB || format == ePixelFormat::PVRTC_RGBA);

		Decompress((AMTC_BLOCK_STRUCT *)iblock, false, 4, 4, 1, pixels);
	}
