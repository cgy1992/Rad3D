...
int etheora_fillyuv(theora_info *ti, yuv_buffer *yuv,
	...
	if(yuv->y_stride < 0) 
		yuv->y = &(yuv->y[ma]); 
	if(!(yuv->y)){
		etheora_fprintf(finfo, "etheora_fillyuv():"
...
void etheora_free_streams_states(etheora_streams_states *ess){
	if(ess){
		...
		{
			ogg_stream_clear(&ess->streams[i]);
		}
...
int etheora_writeheaders(theora_state *ts, 
	...
	if (theora_encode_header (ts, &header_p)){
		etheora_fprintf(finfo, "Can't encode headers.\n");
	...
	if(ogg_stream_pageout(stream, &page)!=1){
		etheora_fprintf(finfo,"Internal Ogg library error.\n");
	...
	if (ret < 0) {
		etheora_fprintf(finfo, "problem pageout-ing. going on.\n");
	}
	...
	while(1){
		...
		if(result ==0 )break;
		etheora_fprintf(finfo, "flush-ing.\n");
		fwrite(page.header,1,page.header_len,fout);
		fwrite(page.body,1,page.body_len,fout);
	}
...
int etheora_writeframe(theora_state *ts, 
	...
	if((ret = theora_encode_YUVin( ts , yuv))){
		if(ret == OC_EINVAL)
			etheora_fprintf(finfo,"Encoder is not ready, or is finished.\n");
		if(ret == -1)
			etheora_fprintf(finfo,"The size of the given frame differs "
	...
	if(! (ret = theora_encode_packetout( ts, iflastframe, &frame_p) )){
		if(ret == 0)
			etheora_fprintf(finfo,"No internal storage exists OR "
			"no packet is ready.\n");
		if(ret == -1)
			etheora_fprintf(finfo,"The encoding process has completed.\n");
		return -1;
	}
	/*printing info..*/
	etheora_fprintf(finfo, "number of packed bytes in the frame: %ld\n",
			frame_p.bytes);
	...
	while((ret = ogg_stream_pageout(stream, page)) > 0){
		/*
		etheora_fprintf(finfo, "pageout-ing frame.\n");
		*/
		fwrite(page->header,1,page->header_len,fout);
		fwrite(page->body,1,  page->body_len,fout);
	}
	if (ret < 0) {
		etheora_fprintf(finfo, "problem pageout-ing frame.\n");
		return -1; 
	}
	return 0; 
}
...
int etheora_flushpage(ogg_stream_state *stream, ogg_page *page, 
	...
	while(1){
		...
		if(result == 0)break;
		etheora_fprintf(finfo, "flush-ing frame.\n");
		fwrite(page->header,1,page->header_len,fout);
		fwrite(page->body,1,  page->body_len,fout);
	}
	return 0;
...
int etheora_buffer_data(FILE *fin, ogg_sync_state *os){
        ...
        int  bytes    = etheora_fread(buffer, 1, buf_size, fin);
...
/*TODO:  improve comments */
	...
	for (i = 0; initiated_stream[i] && i < num_streams; i++){
		...
		switch(a){
			case -1: etheora_fprintf(finfo, "ogg_stream_packetout(packet):"
			...
			case  1: etheora_fprintf(finfo, "ogg_stream_packetout(packet):"
				 ...
				 b = i+1; 
				 break;
			case  0: etheora_fprintf(finfo, "ogg_stream_packetout(packet):"
	...
	if(b) return (rv = b - 1); 
        ...
        do{
                while(ogg_sync_pageout(sync_s, &op) != 1 ){
                        ...
                        if(!a){
				etheora_fprintf(finfo, "No data read from buffer.\n");
				return ETHEORA_NOMOREDATA; 
			}
                        else etheora_fprintf(finfo, "%i bytes read from buffer.\n", a);
                };
		...
		*/
			...
			if(!initiated_stream[i]){
				...
				/*initiated_stream[i]++; */
				if(a) {
					etheora_fprintf(finfo, "ogg_stream_init(): ogg_stream_state"
							" wasn't properly declared.\n");
					return ETHEORA_ERR_BAD_STREAM; 
				}
				else {
					etheora_fprintf(finfo, "stream[%i] initiated.\n", i); 
					rv = 1; 
				}
			}
                }
		...
		for (i = 0; initiated_stream[i] && i < num_streams; i++){
			etheora_fprintf(finfo, "page serial number: %i\n", 
				ogg_page_serialno(&op)); 
			etheora_fprintf(finfo, "stream (%i) serial number: %ld \n", i, 
				(&streams[i])->serialno ); 
			a = ogg_stream_pagein(&streams[i], &op); 
			if(!a) etheora_fprintf(finfo, "ogg_stream_pagein(): The page was"
					" successfully submitted to the bitstream (%i).\n", i);
			else {
				etheora_fprintf(finfo, "ogg_stream_pagein() fails: (%i). This"
			...
			switch(a){
				...
				case  1: rv = i; 
					 b = i+1; 
					 etheora_fprintf(finfo, "ogg_stream_packetout(packet):"
							 " packet was assembled normally.\n");
					 break;
				case  0: etheora_fprintf(finfo, "ogg_stream_packetout(packet):"
							 " there is insufficient data"
...
int etheora_decode_header(theora_info *ti, theora_comment *tc, 
	...
	rv = theora_decode_header(ti, tc, o_packet); 
        if ( rv ) {
                etheora_fprintf(finfo, "theora_decode_header(ogg_packet) can't start: ");
                switch (rv){
                        case OC_BADHEADER:
                                etheora_fprintf(finfo, "OC_BADHEARDER.\n");
                                break;
                        case OC_VERSION:
                                etheora_fprintf(finfo, "OC_VERSION.\n");
                                break;
                        case OC_NEWPACKET:
                                etheora_fprintf(finfo, "OC_NEWPACKET.\n");
                                break;
                        default:
                                etheora_fprintf(finfo, "Reason undocumented"
                                " by libtheora API (return value=%i).\n", rv);

                }
        }
        else etheora_fprintf(finfo, "o_packet show.\n");
	return rv; 
}
	...
	*ess, ogg_sync_state *os, theora_comment *tc,  FILE *finfo){
	...
	/*decode packets and decode headers untill we get the
	  nh == 3 (3 -> number of theora headers).*/
        for(nh = 0; nh < 3; ){
                etheora_fprintf(finfo, ".................... header. %i \n", nh);
			...
			return ETHEORA_NOT_ENOUGH_PACK; 
                ...
                if(!etheora_decode_header(ti, tc, &o_pack, finfo)){
                        etheora_fprintf(finfo, "header show (%i/3).\n", ++nh);
                }
        }
	return 0; 
}
...
int etheora_decode_frame(FILE *fin, theora_state *ts, etheora_streams_states *ess,
	...
	if(a) { 
		etheora_fprintf(finfo,"theora_decode_packetin(): frame does not"
			" contain encoded video data.\n" );
		return ETHEORA_ERR_DEC_NOVIDEODATA; 
	}
	a = theora_decode_YUVout(ts, yuvb);
	if(a) {
		etheora_fprintf(finfo,"theora_decode_YUVout():"
			" undocummented error\n.");
